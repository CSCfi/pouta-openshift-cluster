...
@test "test deployment with a pvc on openshift (standard-rwo storage class)" {
  ...
  run /usr/lib64/nagios/plugins/check_oso_deploy/check_oso_deploy.bash --use_pvc --timeout 120 --storage_class standard-rwo
  [ $status -eq 0 ]
}

@test "test deployment with a pvc on openshift (glusterfs-storage storage class)" {
  skip_if_sc_is_inactive glusterfs-storage
  run /usr/lib64/nagios/plugins/check_oso_deploy/check_oso_deploy.bash --use_pvc --pvc_delay 10 --timeout 120 --storage_class glusterfs-storage
  [ $status -eq 0 ]
}
